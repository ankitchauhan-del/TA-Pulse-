...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.status(500).json({ error: 'Failed to read narrative', detail: String((e && e.message) || e) });
    }
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      return res.status(500).json({ error: 'Failed to save narrative', detail: String((e && e.message) || e) });
    }
  }
